...
BuildNodeOperatorData[nAfun_, nodes_, endpoints_, endpointOperators_, prec_, precA_, parallelQ_, firstIndex_ : 2] := Module[
  {m, buildOne, taskCount},
  m = Length[nodes] - 1;
...
SpectralPropagateSequential[nAfun_, y0_, {x0_, x1_}, opts : OptionsPattern[SpectralPropagate]] := Module[
  {
    ...
    endpointInfo, parallelQ, nodeParallelQ, nodeSubBlocks, source, leftEndpointRegularized, zValues
  },
  ...
  (* Endpoint extraction and node matrix evaluation are the only parallel stages. *)
  parallelQ = CHESSResolveParallelQ[OptionValue["ParallelEvaluation"], m, OptionValue["ParallelKernels"]];
  ...
  nodeParallelQ = parallelQ && (m + 2 - If[TrueQ[leftEndpointRegularized], 1, 2] >= 8);
